...
func ModifyNote() {
	...
	HttpInvoke(api2.URI_ModifyNote, `{"id":1003,"is_anonymous":false,"title":"","content":""}`) // 这里借用create接口创建的note
	...
	HttpInvoke(api2.URI_ModifyNote, `{"id":1003,"is_anonymous":false,"title":"123","content":"456"}`)

	TestCase("success")
	res := HttpInvoke(api2.URI_ModifyNote, `{"id":1003,"is_anonymous":true,"title":"123","content":"456"}`)
	log.Println(res)
