...
- 完成后会部署到公网，如果[公网地址](这里填写公网地址)不可用，多半是我买的云服务器到期了 ^_^
...
todo：拟于doc目录下创建design文档，包含如下内容：
- 目录：介绍项目的目录结构
- 开发：介绍代码过程，例如orm、dao使用的都是gorm；前后端传递消息的格式规范使用的是自研工具gocts；程序启动过程等，考虑配图
